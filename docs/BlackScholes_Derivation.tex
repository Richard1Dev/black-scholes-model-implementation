...
In the simple case of selling a vanilla call option, we can be at risk of losing money when the asset is in-the-money. For every \$1 the underlying gains, we would lose \$1, with potentially unlimited loss. But if we own 1 unit of the underlying then, for every \$1 lost from the option, we would gain that \$1 back from the underlying. If the asset drops out-the-money then we sell the underlying so we don't lose money on that either. This is {\emph{dynamic hedging}}.
